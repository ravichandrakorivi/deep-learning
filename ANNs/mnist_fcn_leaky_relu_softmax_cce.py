...
# Initialize biases
# With ReLU/Leaky ReLU, large random biases can:
# push activations too positive initially
# make training slightly unstable
biases = [np.zeros((size, 1)) for size in sizes[1:]]

# He Initialization for weight
# Best for ReLU 
weights = [
    rng.normal(0, np.sqrt(2 / curr), size=(next, curr))
...
def backprop(x, y):
    ...
    for i, (b, w) in enumerate(zip(biases, weights)):
        ...
        if i == len(weights) - 1:
            ...
            activation = leaky_relu(z)      # HIDDEN LAYERS
...
def evaluate(data):
    ...
    for x, y in data:
        ...
        for i, (b, w) in enumerate(zip(biases, weights)):
            if i == len(weights) - 1:
                output = softmax(np.dot(w, output) + b)
            else:
                output = leaky_relu(np.dot(w, output) + b)    
...
def compute_loss(data):
    ...
    for x, y in data:
        ...
        loss += -np.sum(y * np.log(output + 1e-9))  # avoid log(0)
...
for epoch in range(epochs):
    ...
    train_acc = evaluate(training_data)
    val_acc = evaluate(validation_data)
    val_loss = compute_loss(validation_data)
    print(f"Epoch {epoch+1}: train_Acc = {train_acc}/{len(training_data)}, val_Acc = {val_acc}/{len(validation_data)}, Loss = {val_loss:.4f}")
